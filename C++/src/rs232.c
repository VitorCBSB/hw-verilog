...
char comports[38][16] = { "/dev/ttyS0", "/dev/ttyS1", "/dev/ttyS2",
		...
		"/dev/ircomm1", "/dev/cuau0", "/dev/cuau1", "/dev/cuau2", "/dev/cuau3",
...
int RS232_OpenComport(int comport_number, int baudrate, const char *mode) {
	...
	if (Cport[comport_number] == INVALID_HANDLE_VALUE) {
		printf("unable to open comport: error code %ld\n", GetLastError());
		return (1);
	}

	/* set input and output buffers to 4096 */
	if (!SetupComm(Cport[comport_number], 4096, 4096)) {
		printf("unable to change input/output buffers: error code %ld.\n",
				GetLastError());
		return 1;
	}
